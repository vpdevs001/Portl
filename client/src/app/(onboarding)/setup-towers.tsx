import { useCreateTower, useTowers } from '@/hooks/use-society';
import { useRouter } from 'expo-router';
import { useState } from 'react';
import { ActivityIndicator, Pressable, ScrollView, Text, TextInput, View } from 'react-native';

export default function SetupTowers() {
    ...
    <ScrollView className="flex-1 bg-background px-6 py-12">
      ...
      <View className="mb-8 mt-4">
        ...
        <Text className="text-sm font-sans text-foreground-secondary leading-5">
          Add the individual buildings, wings, or blocks that make up your estate. You need at least one tower to assign flats.
        </Text>
      </View>
        ...
        <View className="flex-row gap-3">
          ...
          >
            {createTowerMutation.isPending ? (
              ...
              <Text className="text-primary-foreground font-sans-semibold text-sm">Add</Text>
            )}
          </Pressable>
        </View>
      </View>
      ...
      <View className="mb-10">
        ...
        {isLoading ? (
          ...
          <View className="gap-2">
            {towers.map((tower) => (
              <View key={tower.id} className="p-4 bg-card border border-border rounded-xl flex-row justify-between items-center">
