...
import type { Flat, Society } from './use-society';
...
export function useSearchUsers(q: string) {
  return useQuery({
    ...
    queryFn: () => apiRequest<UserSearchItem[]>(`/api/invites/search-users?q=${encodeURIComponent(q)}`),
...
export function useSentInvites(status?: 'pending' | 'accepted' | 'rejected' | 'cancelled') {
  return useQuery({
    ...
    queryFn: () => apiRequest<SocietyInvite[]>(`/api/invites/sent${status ? `?status=${status}` : ''}`)
  });
}
...
export function useRespondInvite() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      // note in use-society.ts's useCreateSociety. The screen calling this
      // mutation must call authClient.useSession().refetch() itself after
      // an 'accept', so societyId/role propagate into the navigation gate.
